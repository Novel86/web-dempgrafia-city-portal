(() => {
	...
	let formValidate = {
		...
		validateInput(formRequiredItem) {
			...
			if ("email" === formRequiredItem.dataset.required) {
				...
				this.addError(formRequiredItem);
				error++;
			} else this.removeError(formRequiredItem); else if ("login" === formRequiredItem.dataset.required) if (this.loginTest(formRequiredItem)) {
				this.addError(formRequiredItem);
				error++;
			} else this.removeError(formRequiredItem); else if ("password" === formRequiredItem.dataset.required) if (this.passTest(formRequiredItem)) {
				this.addError(formRequiredItem);
				error++;
			} else this.removeError(formRequiredItem); else if ("passRepeat" === formRequiredItem.dataset.required) if (this.passRepeatTest(formRequiredItem)) {
				this.addError(formRequiredItem);
				error++;
	...
	window.onload = function () {
		...
		if (elements) {
			let removeActive = () => {
				let elemActive = Array.from(document.querySelectorAll("._active"));
				for (let elAc of elemActive) elAc.className = "";
			};
			for (let elem of elements) elem.onclick = () => {
				removeActive();
				elem.classList.toggle("_active");
				elem.querySelector("td:nth-child(1)").innerHTML, elem.querySelector("td:nth-child(2)").innerHTML,
					elem.querySelector("td:nth-child(3)").innerHTML;
				let titleTable = elem.querySelector("td:nth-child(4)").innerHTML, discriptTable = elem.querySelector("td:nth-child(5)").innerHTML;
	...
	let category = document.querySelector('button[data-value="добавить категорию"]');
	category.addEventListener('click', () => {
		addCategory();
	})
	function addCategory() {
		let newCategoryTitle = prompt('Ведите назавание категории');
		console.log(newCategoryTitle);
		// учим ajax и все что связано. потом добавляем категорию. на завтра тебе много дел по этой задаче
	}
})();